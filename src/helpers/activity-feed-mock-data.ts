import { INITIAL_PROTOCOLS, type Protocol } from "helpers/greenwood-mock-data"
...
import {
  ...
  previewHoverAccentBazar,
  previewHoverAccentForMiniAppFork,
  previewHoverAccentLlamaland,
} from "helpers/feed-mini-app-previews"
...
export type GlobalFeedItem = {
  ...
  /** Mock wallet shown abbreviated in the feed (deterministic from fork). */
  ...
  previewHtml: string
  /** Dominant preview hue for feed card hover (synced with mock generators; not pixel-sampled). */
  previewHoverAccent: string
  ...
  for (const protocol of protocols) {
    for (const fork of protocol.forks) {
      ...
      items.push({
        ...
        builderWallet: mockEthereumAddressFromSeed(`wallet:${fork.id}`),
        createdAt: fork.createdAt,
        score: scoreForFork(fork.id),
        previewHtml: buildMiniAppPreviewHtml(fork.id, protocol.name),
        previewHoverAccent: previewHoverAccentForMiniAppFork(fork.id),
  ...
  return items.map((item) => {
    ...
    if (item.forkId === "p-jaja-research-f2") {
      ...
      return {
        ...
        previewHoverAccent: previewHoverAccentLlamaland(),
      }
    }
    if (item.forkId === "p-jaja-research-f1") {
      return {
        ...
        previewHoverAccent: previewHoverAccentBazar(),
        cardTitle: "Bazar",
      }
    }
