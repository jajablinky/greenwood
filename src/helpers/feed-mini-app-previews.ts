...
/**
 * CSS color for feed card hover tint. Mirrors hue picks in `buildMiniAppPreviewHtml`.
 * Sandboxed preview iframes cannot be sampled from the parent, so this stays in sync
 * with the mock HTML generators instead of reading pixels.
 */
export function previewHoverAccentForMiniAppFork(forkId: string): string {
  const h = hashString(forkId)
  const variant = h % 7
  const hue = h % 360
  const hue2 = (h * 17 + 40) % 360
  switch (variant) {
    ...
    case 4:
      return `hsl(${hue2} 30% 45%)`
    case 5:
      return `hsl(${hue} 45% 32%)`
    default:
      return "hsl(142 71% 33%)"
  }
}

/** Bazar-style marketplace mock — teal listing accent. */
export function previewHoverAccentBazar(): string {
  return "hsl(168 76% 32%)"
}

/** Llamaland mock — sky from the hero gradient. */
export function previewHoverAccentLlamaland(): string {
